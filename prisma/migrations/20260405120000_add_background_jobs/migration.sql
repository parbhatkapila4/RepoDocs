DO $$ BEGIN CREATE TYPE "BackgroundJobKind" AS ENUM ('readme_regen', 'docs_regen');
EXCEPTION
WHEN duplicate_object THEN NULL;
END $$;
DO $$ BEGIN CREATE TYPE "BackgroundJobStatus" AS ENUM ('running', 'completed', 'failed');
EXCEPTION
WHEN duplicate_object THEN NULL;
END $$;
...
DO $$ BEGIN IF NOT EXISTS (
  SELECT 1
  FROM pg_constraint
  WHERE conname = 'BackgroundJob_userId_fkey'
) THEN
...
DO $$ BEGIN IF NOT EXISTS (
  SELECT 1
  FROM pg_constraint
  WHERE conname = 'BackgroundJob_projectId_fkey'
) THEN
ALTER TABLE "BackgroundJob"
