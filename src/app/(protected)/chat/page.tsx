...
const models = [
  ...
  {
    id: "repodoc-guidance",
    name: "RepoDoc AI (Guidance)",
    description: "Files, order, risks, tests—no code unless asked",
  },
];
...
export default function ChatPage() {
  ...
  const userName =
    user?.firstName || user?.emailAddress?.split("@")[0] || "Developer";
    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-[#0a0a0f] px-4">
        <div className="text-center p-6 sm:p-8 max-w-md">
          <div className="w-16 h-16 sm:w-20 sm:h-20 mx-auto mb-4 sm:mb-6 rounded-2xl bg-linear-to-br from-indigo-500 to-purple-600 flex items-center justify-center shadow-lg shadow-indigo-500/25">
          ...
          <p className="text-sm sm:text-base text-gray-400 leading-relaxed">
            Please select a project from the sidebar to start chatting with your
            codebase
          </p>
        </div>
      </div>
  ...
  return (
    <div className="flex flex-col h-screen bg-[#0a0a0f] overflow-hidden">
      <div className="shrink-0 pt-3 sm:pt-4 pb-2 px-3 sm:px-4">
        <div className="flex items-center justify-between sm:justify-center sm:relative gap-2">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              ...
              >
                <div className="w-5 h-5 sm:w-6 sm:h-6 rounded-lg bg-linear-to-br from-indigo-500 to-purple-600 flex items-center justify-center">
            ...
            <DropdownMenuContent
              align="start"
              className="w-56 bg-[#16161d] border-white/10 shadow-2xl rounded-xl p-1 sm:align-center"
            >
              {models.map((model) => (
                <DropdownMenuItem
                  key={model.id}
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        {messages.length === 0 ? (
          <div className="flex-1 flex flex-col items-center justify-center px-3 sm:px-4 pb-4 sm:pb-8">
            <div className="relative mb-6 sm:mb-8 w-32 h-32 sm:w-40 sm:h-40 md:w-48 md:h-48">
              ...
              <div className="absolute inset-4 sm:inset-6 rounded-full bg-linear-to-br from-purple-600/20 via-indigo-600/20 to-blue-600/20 blur-xl animate-pulse" />
              <div
                className="absolute inset-6 sm:inset-8 rounded-full bg-linear-to-tr from-blue-500/30 via-purple-500/30 to-pink-500/30 blur-lg animate-pulse"
                style={{ animationDelay: "0.5s" }}
              />

              <div className="absolute inset-8 sm:inset-10 md:inset-12 rounded-full bg-linear-to-br from-indigo-500 via-purple-600 to-blue-600 shadow-2xl shadow-purple-500/50 flex items-center justify-center overflow-hidden">
                <div className="absolute inset-0 bg-linear-to-t from-transparent via-white/10 to-white/20 rounded-full" />

                <div className="absolute w-full h-full">
                  <div
                    className="absolute top-1/4 left-1/4 w-1 h-1 rounded-full bg-white/60 animate-ping"
                    style={{ animationDuration: "2s" }}
                  />
                  <div
                    className="absolute top-1/3 right-1/3 w-0.5 h-0.5 rounded-full bg-white/40 animate-ping"
                    style={{
                  ...
                  />
                  <div
                    className="absolute bottom-1/3 left-1/3 w-0.5 h-0.5 rounded-full bg-white/50 animate-ping"
                    style={{ animationDuration: "3s", animationDelay: "1s" }}
                  />
                </div>

                <svg
                  className="w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12 text-white relative z-10 drop-shadow-lg"
                  viewBox="0 0 24 24"
                  fill="none"
                  stroke="currentColor"
                  strokeWidth="1.5"
              ...
              <div
                className="absolute -top-2 left-1/4 w-1.5 h-1.5 rounded-full bg-purple-400/60 animate-bounce"
                style={{ animationDuration: "3s" }}
              />
              <div
                className="absolute -bottom-2 right-1/4 w-1 h-1 rounded-full bg-blue-400/60 animate-bounce"
                style={{ animationDuration: "2.5s", animationDelay: "0.5s" }}
              />
              <div
                className="absolute top-1/4 -right-2 w-1.5 h-1.5 rounded-full bg-indigo-400/60 animate-bounce"
                style={{ animationDuration: "2s", animationDelay: "1s" }}
              />
              <div
                className="absolute bottom-1/4 -left-2 w-1 h-1 rounded-full bg-cyan-400/60 animate-bounce"
              ...
              <form onSubmit={handleSubmit} className="relative">
                <div className="relative bg-white/5 backdrop-blur-xl rounded-xl sm:rounded-2xl shadow-2xl shadow-black/20 border border-white/10 overflow-hidden">
                  <div className="flex items-start p-2 sm:p-3">
                    <div className="shrink-0 p-2 sm:p-2.5 mt-0.5">
                  ...
                  <div className="flex items-center gap-1.5 sm:gap-2 px-3 sm:px-5 py-3 sm:py-4 border-t border-white/5 overflow-x-auto scrollbar-hide">
                    {actionButtons.map((action, idx) => (
                      <button
                        ...
                        className="inline-flex items-center gap-1 sm:gap-1.5 px-2 sm:px-3 py-1.5 text-xs sm:text-sm font-medium text-gray-400 hover:text-white bg-white/5 hover:bg-white/10 rounded-lg transition-colors border border-white/5 hover:border-white/10 whitespace-nowrap shrink-0"
                      >
                        <action.icon
                          className={`w-3.5 h-3.5 sm:w-4 sm:h-4 ${action.color}`}
                        />
          ...
          >
            <div className="max-w-4xl mx-auto px-3 sm:px-4 py-4 sm:py-6 space-y-4 sm:space-y-6">
              {messages.map((message) => (
                ...
                >
                  <div
                    className={`max-w-[92%] sm:max-w-[85%] ${message.role === "user"
                        ? "bg-linear-to-r from-indigo-600 to-purple-600 text-white rounded-2xl rounded-tr-md px-3 sm:px-4 py-2.5 sm:py-3 shadow-lg shadow-indigo-500/20"
                        ...
                        components={{
                          code({
                            ...
                            return !inline && match ? (
                              ...
                              <code
                                className={`${className} bg-white/10 text-indigo-300 px-1 sm:px-1.5 py-0.5 rounded text-xs sm:text-sm break-all`}
                                {...props}
                              >
                                {children}
                              </code>
                          ...
                          {message.sources.map((source, idx) => (
                            ...
                            >
                              <div className="flex items-center justify-between gap-2 mb-1">
                                <code className="text-indigo-400 font-medium truncate text-[10px] sm:text-xs">
                                  {source.fileName}
                                </code>
                                <span className="shrink-0 text-[10px] sm:text-xs text-gray-500 bg-white/5 px-1 sm:px-1.5 py-0.5 rounded">
        ...
        {messages.length > 0 && (
          <div className="shrink-0 border-t border-white/5 bg-[#0a0a0f]/80 backdrop-blur-xl p-2.5 sm:p-4">
              ...
              <div className="relative bg-white/5 backdrop-blur-xl rounded-xl shadow-2xl shadow-black/20 border border-white/10 overflow-hidden">
                <div className="flex items-center p-1.5 sm:p-2">
                  ...
                  <Button
                    ...
                    className="shrink-0 m-1 bg-linear-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 text-white rounded-lg h-8 w-8 p-0 shadow-lg shadow-indigo-500/25"
                  >
                    {isLoading ? (
                      <Loader2 className="w-4 h-4 animate-spin" />
                    ) : (
